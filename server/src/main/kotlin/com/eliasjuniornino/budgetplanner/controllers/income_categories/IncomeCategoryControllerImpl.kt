...
class IncomeCategoryControllerImpl(
    ...
    override suspend fun index(context: RoutingContext) = context.apply {
        ...
        val categories = incomeCategoriesRepository.list(user.id)
    ...
    override suspend fun indexByCategory(context: RoutingContext) = context.apply {
        ...
        val user = getAuthenticatedUserOrRespondError() ?: return@apply
        val categories = incomeCategoriesRepository.list(user.id, parentId)
        call.respond(categories.map { it.toDTO() })
    }
    ...
    override suspend fun view(context: RoutingContext) = context.apply {
        ...
        val category = incomeCategoriesRepository.get(user.id, categoryId)
        if (category == null) {
    ...
    override suspend fun create(context: RoutingContext) = context.apply {
        ...
        if (request.name != null && incomeCategoriesRepository.existsByName(user.id, request.name!!)) {
            call.respond(
        ...
        val createdCategory = incomeCategoriesRepository.store(user.id, newCategory)
    ...
    override suspend fun update(context: RoutingContext) = context.apply {
        ...
        val category = incomeCategoriesRepository.get(user.id, request.id)
        if (category == null) {
            call.respond(HttpStatusCode.NotFound, mapOf("message" to "Category not found"))
            return@apply
        }
        ...
        val updatedCategory = incomeCategoriesRepository.update(user.id, category)
    ...
    override suspend fun delete(context: RoutingContext) = context.apply {
        ...
        val deleted = incomeCategoriesRepository.delete(user.id, categoryId)
        if (!deleted) {
